...
const upload = multer({dest: 'nods/images/originalImg/'})
const fs = require('fs');
...
app.get('/api/images/:type/:apikey',async (req,res) =>{
	...
	if (!uuidAPIKey.isAPIKey(apikey) || !uuidAPIKey.check(apikey, key.uuid)) {
		...
		console.log(__dirname+'nods/images/examinedImg/'+type+ ".jpeg");
		fs.exists(__dirname+'nods/images/examinedImg/'+type+ ".jpeg", function (exists){
			if(exists)
			{
				fs.readFile(__dirname+'nods/images/examinedImg/'+type + ".jpeg", function(err,data){
					res.end(data);
				});
			}
			else
...
app.post('/api/users/:type/post/:type2/:apikey',upload.single('image'), async(req, res) =>{
	...
	if (!uuidAPIKey.isAPIKey(apikey) || !uuidAPIKey.check(apikey, key.uuid)) {
		...
		var commd = "python3 yolov5/client.py " + examineFile + " " + plant_Code;
		...
		exec(commd, (err,out,stderr) => {//child process 생성
			...
			fs.exists(__dirname+'/nods/images/examinedImg/' + fileName + ".jpeg", function (exists){
				if(exists)
				{
